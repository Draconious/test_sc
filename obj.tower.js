module.exports = {
    defend: function (myRoomName) {
        ...
        var hostileHealers = Game.rooms[myRoomName].find(FIND_HOSTILE_CREEPS, {
            filter: (s) => s.getActiveBodyparts(HEAL) > 0 && MY_FRIENDS.indexOf(creep.owner.username) === -1
        });
        ...
        var hostileAttackers = Game.rooms[myRoomName].find(FIND_HOSTILE_CREEPS, {
            filter: (s) => (s.getActiveBodyparts(ATTACK) > 0 || s.getActiveBodyparts(RANGED_ATTACK) > 0) && MY_FRIENDS.indexOf(creep.owner.username) === -1
        });
